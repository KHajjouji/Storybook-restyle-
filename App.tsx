...
const App: React.FC = () => {
  ...
  const handleExportProjectFile = () => {
    ...
    // Use window.Blob to avoid potential shadowing issues from external libraries
  ...
  const handleCharImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    // Explicitly cast to File to resolve unknown type issues
    ...
    // Ensure the argument passed is recognized as a Blob
  ...
  const handleRestyleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    // Explicitly cast to File[] to resolve unknown type issues during Array.from
    ...
    files.forEach(f => {
      ...
      // Ensure the argument is seen as a Blob
    ...
    try {
      ...
      if (settings.mode === 'upscale' && p.originalImage) {
        ...
        if (p.originalImage) {
          ...
          result = await refineIllustration(p.originalImage, outpaintingInstruction, others, p.isSpread);
        } else {
          result = await restyleIllustration(undefined, outpaintingInstruction, undefined, undefined, settings.characterReferences, [], true, false, p.isSpread, settings.masterBible);
        }
      }
  ...
  /**
   * Targeted corrective edits for a single illustration frame.
   */
  const handleRefineScene = async (pageId: string) => {
    ...
    try {
      ...
      // Pass empty reference images for targeted text-only instruction unless user explicitly adds them
      const res = await refineIllustration(targetImg, refinePrompt, [], p.isSpread);
  ...
  const renderStep = () => {
    switch (currentStep) {
          ...
          <div className="max-w-6xl mx-auto py-16 space-y-16 animate-in fade-in duration-700 pb-32">
            ...
            <div className="space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <button onClick={() => { setSettings({...settings, mode: 'create'}); setCurrentStep('script'); }} className="p-8 bg-white border-2 border-slate-100 rounded-[3rem] text-left hover:border-indigo-600 hover:scale-[1.05] transition-all group shadow-sm">
      ...
      case 'script':
        return (
          <div className="max-w-4xl mx-auto py-16 space-y-12 animate-in slide-in-from-bottom duration-500">
            <div className="text-center space-y-4"><h2 className="text-5xl font-black">Script-to-Book</h2><p className="text-slate-500 text-lg">Auto-Storyboard your full narrative into production frames.</p></div>
            <textarea className="w-full h-96 bg-white border-2 border-slate-100 rounded-[3rem] p-10 font-medium text-lg outline-none shadow-inner leading-relaxed" placeholder="Write or paste your story here..." value={fullScript} onChange={e => setFullScript(e.target.value)} />
            <button disabled={isParsing || !fullScript} onClick={handlePlanStory} className="w-full py-8 bg-indigo-600 text-white rounded-[2.5rem] font-black text-2xl shadow-xl flex items-center justify-center gap-4 hover:scale-[1.02] transition-all disabled:opacity-50">
      ...
      case 'direct-upscale':
        return (
          <div className="max-w-4xl mx-auto py-16 space-y-12 animate-in slide-in-from-bottom duration-500">
            <div className="text-center space-y-4"><h2 className="text-5xl font-black">4K Master Enhancement</h2><p className="text-slate-500 text-lg">Restore and upscale illustrations to print-quality 4K.</p></div>
            <div onClick={() => restyleInputRef.current?.click()} className="aspect-[2/1] bg-white border-4 border-dashed border-slate-200 rounded-[4rem] flex flex-col items-center justify-center cursor-pointer hover:border-indigo-600 transition-all group shadow-sm bg-indigo-50/10">
      ...
      case 'characters':
        return (
          <div className="max-w-7xl mx-auto py-12 space-y-12 pb-40 px-8">
            <div className="text-center space-y-4"><h2 className="text-5xl font-black">Character Continuity Lab</h2><p className="text-slate-500 text-lg">Define and upload your cast to lock visual features.</p></div>
                ...
                <div key={char.id} className="bg-white p-8 rounded-[4rem] border-2 border-slate-100 shadow-xl space-y-6 group transition-all">
                  ...
                  <div className="space-y-4 text-center">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => { setActiveCharId(char.id); charImageInputRef.current?.click(); }} className="flex-1 py-3 bg-indigo-50 text-indigo-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all">Upload</button>
                      ...
                      })} className="flex-1 py-3 bg-slate-50 text-slate-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all">Design</button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur-3xl p-12 z-50 flex justify-center gap-6">
               <button onClick={() => setCurrentStep('cover-master')} className="bg-indigo-600 text-white px-24 py-7 rounded-[3rem] font-black text-2xl flex items-center gap-4 shadow-2xl hover:scale-105 transition-all">CONTINUE TO COVER DESIGN</button>
            </div>
          </div>
        );
            ...
            <div className="flex flex-col md:flex-row gap-12 items-start">
              <div className="flex-1 space-y-8 sticky top-36">
                <div className="flex justify-between items-end">
                  <h2 className="text-5xl font-black">Production Setup</h2>
                ...
                <div className="bg-white border-2 border-slate-100 rounded-[4rem] p-10 shadow-2xl space-y-8">
                  <div className="space-y-4">
                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-indigo-600">Reformat Ratio</h3>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-indigo-600">Global Production Prompt</h3>
                ...
                {pages.map((p, idx) => (
                  <div key={p.id} onClick={() => setSelectedForProduction(prev => { const n = new Set(prev); n.has(p.id) ? n.delete(p.id) : n.add(p.id); return n; })} className={`bg-white p-6 rounded-[3.5rem] border-4 transition-all cursor-pointer group relative ${selectedForProduction.has(p.id) ? 'border-indigo-600 shadow-2xl scale-[1.02]' : 'border-slate-50 opacity-60 grayscale hover:opacity-100 hover:grayscale-0'}`}>
                    ...
                    <div className="aspect-[4/3] bg-slate-100 rounded-[2.5rem] overflow-hidden shadow-inner">
                      {p.originalImage && <img src={p.originalImage} className="w-full h-full object-cover" />}
                    </div>
                  </div>
                ))}
              </div>
      ...
      case 'generate':
        return (
          <div className="max-w-7xl mx-auto py-12 space-y-12 animate-in fade-in duration-500 pb-60 px-8">
            <div className="text-center"><h2 className="text-5xl font-black text-slate-900">Final Processing</h2><p className="text-slate-500 text-xl font-medium tracking-tight">Industrializing your illustration frames.</p></div>
              ...
              {pages.map((p, idx) => (
                <div key={p.id} className="bg-white rounded-[5rem] border-4 border-slate-50 overflow-hidden shadow-2xl relative group">
                  ...
                  <div className="p-12 space-y-6">
                    <div className="flex justify-between items-center"><span className="text-xs font-black text-slate-400 uppercase tracking-widest">Illustration {idx+1}</span><div className="flex gap-2"><button disabled={p.status === 'processing'} onClick={() => setActiveRefineId(p.id)} className="bg-indigo-50 text-indigo-600 px-4 py-2 rounded-xl text-[10px] font-black">QUICK TWEAK</button></div></div>
                    {activeRefineId === p.id && (
                      <div className="bg-indigo-50 p-6 rounded-[2rem] space-y-4">
                        ...
                        <div className="flex justify-end gap-2"><button onClick={() => setActiveRefineId(null)} className="text-[10px] font-bold text-slate-400 px-4 py-2">CANCEL</button><button onClick={() => handleRefineScene(p.id)} className="bg-indigo-600 text-white px-6 py-2 rounded-xl text-[10px] font-black">APPLY</button></div>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="fixed bottom-0 inset-x-0 bg-slate-900 text-white p-14 z-50 rounded-t-[7rem] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-12">
                <div className="flex items-center gap-8"><div className="w-24 h-24 bg-white/10 rounded-full flex items-center justify-center text-indigo-400 font-black text-3xl shadow-inner">{stats.progress}%</div><span className="text-3xl font-black uppercase tracking-tighter">Production Completion</span></div>
                <div className="flex gap-8">
                  ...
                  <button disabled={stats.completed === 0 || isProcessing} onClick={() => generateBookPDF(pages, settings.exportFormat, projectName, false, settings.estimatedPageCount, settings.spreadExportMode)} className="bg-indigo-600 px-24 py-10 rounded-[4rem] font-black text-3xl flex items-center gap-8 hover:bg-indigo-500 transition-all disabled:opacity-30 shadow-2xl scale-105"><Download size={48} /> FULL EXPORT</button>
                </div>
            </div>
          </div>
        );
      ...
      case 'upload':
        return (
          <div className="max-w-4xl mx-auto py-12 space-y-8 animate-in slide-in-from-bottom duration-500 px-8">
            <div className="text-center space-y-4"><h2 className="text-5xl font-black text-slate-900">Project Workspace</h2><p className="text-slate-500 text-xl font-medium">Upload illustrations to start the Production dashboard.</p></div>
            ...
            <div className="flex justify-center gap-6"><button onClick={() => setCurrentStep('landing')} className="text-slate-400 font-bold hover:text-slate-600">Cancel</button></div>
          </div>
        );
