...
import {
  ...
  refineLayeredIllustration,
  generateLayeredCover,
  separateIllustrationIntoLayers,
  selectStoryFont,
  generateTextLayerForIllustration,
} from "./geminiService";
...
const SpreadGuide = ({
  ...
  const actualGutter = format.startsWith("KDP_")
    ? getInsideMargin(pageCount)
    : config.baseGutter;
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-20">
      ...
      />
      <div className="absolute top-2 right-2 text-red-500/60 font-black text-[6px] uppercase tracking-widest">
        Bleed Zone
      </div>
      ...
      {isSpread && (
        <>
          ...
          <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-red-500 text-white text-[8px] font-black px-2 py-0.5 rounded uppercase tracking-widest shadow-lg whitespace-nowrap">
            Gutter / Fold
          </div>
        </>
      )}
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-indigo-600 text-white text-[8px] font-black px-2 py-0.5 rounded uppercase tracking-widest shadow-lg whitespace-nowrap">
        Safe Text Area
      </div>
    </div>
  );
...
const LayerManager = ({
  ...
  return (
    <div className="mt-6 space-y-3">
      <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-4">
        Production Layers
      </h4>
...
const PREDEFINED_STYLES = [
  {
    ...
    image:
      "https://images.unsplash.com/photo-1579783902614-a3fb3927b6a5?auto=format&fit=crop&q=80&w=400&h=400",
    prompt:
      "classic children’s book illustration, soft painterly textures, whimsical, rounded shapes, warm inviting lighting, highly detailed but stylized",
  },
  {
    id: "style-2",
    ...
    prompt:
      "vibrant children’s book illustration, bright bold colors, playful and energetic, flat shading with subtle gradients, cute proportions",
  },
  {
    id: "style-3",
    name: "Watercolor Magic",
    image:
      "https://images.unsplash.com/photo-1580136608260-4eb11f4b24fe?auto=format&fit=crop&q=80&w=400&h=400",
    prompt:
      "whimsical watercolor children’s book illustration, visible paper texture, soft color bleeds, magical atmosphere, loose brushstrokes, dreamy lighting",
  },
  {
    id: "style-4",
    name: "Soft Pastel",
    ...
    prompt:
      "soft pastel children’s book illustration, gentle glow lighting, warm pastel palette, minimal outlines, soothing and calm mood, dreamy",
  },
];
...
const getAvailableSteps = (mode: AppMode): { id: Step; label: string }[] => {
  switch (mode) {
    ...
    case "upscale":
      return [{ id: "direct-upscale", label: "Upscale" }];
    case "niche-research":
      return [{ id: "niche-research", label: "Research" }];
    case "cover-designer":
      return [
        { id: "characters", label: "Cast" },
...
const App: React.FC = () => {
  ...
  const [selectedCoverCharIds, setSelectedCoverCharIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  // Production Config
  const [globalFixPrompt, setGlobalFixPrompt] = useState(
    "Keep character facial features and clothing consistent with reference images.",
  );
  const [targetAspectRatio, setTargetAspectRatio] = useState<
    "1:1" | "4:3" | "16:9" | "9:16"
  >("4:3");
  const [targetResolution, setTargetResolution] = useState<"1K" | "2K" | "4K">(
    "1K",
  );
  ...
  const [fixMode, setFixMode] = useState<
    "targeted" | "outpaint" | "separate-layers"
  >("targeted");
  const [outpaintPos, setOutpaintPos] = useState<"left" | "center" | "right">(
    "center",
  );
  ...
  const [retargetSourceImage, setRetargetSourceImage] = useState<string | null>(
    null,
  );
  ...
  const renderProjectThumbnails = (
    proj: Project,
    className: string = "w-full h-48 flex flex-wrap bg-slate-200",
  ) => {
    ...
    if (validImages.length === 0 && proj.coverImage)
      validImages.push(proj.coverImage);
    if (validImages.length === 0 && proj.thumbnail)
      validImages.push(proj.thumbnail);
      ...
      return (
        <div className={className}>
          {validImages.slice(0, 4).map((img, i) => (
            <div
              key={i}
              className={`relative ${validImages.length === 1 ? "w-full h-full" : validImages.length === 2 ? "w-1/2 h-full" : validImages.length === 3 ? (i === 0 ? "w-full h-1/2" : "w-1/2 h-1/2") : "w-1/2 h-1/2"} border-white border-[0.5px]`}
            >
              <img
                src={img}
    ...
    return (
      <div
        className={
          className + " items-center justify-center text-slate-300 flex"
        }
      >
        <BookOpen size={40} />
      </div>
        ...
        const unsubscribeProfile = onSnapshot(
          ...
          (error: any) => {
            if (
              ...
              console.warn(
                "Profile snapshot quota exceeded, using local fallback.",
              );
            }
  ...
  const [settings, setSettings] = useState<AppSettings>({
    mode: "restyle",
    targetStyle:
      "soft vibrant children’s storybook illustration, painterly, rounded shapes, big expressive eyes, gentle glow lighting, warm pastel palette, minimal outlines",
  ...
  const handleSaveProject = async () => {
    const project: Project = {
      ...
      pages,
      thumbnail:
        pages.find((p) => p.processedImage)?.processedImage ||
        pages[0]?.originalImage,
      currentStep,
      fullScript,
      activityScript,
      nicheTopic,
  ...
  useEffect(() => {
    ...
    settings.characterReferences.forEach((c) => {
      if (
        c.name &&
        projectContext.toLowerCase().includes(c.name.trim().toLowerCase()) &&
        !newIds.has(c.id)
      ) {
        newIds.add(c.id);
        changed = true;
  ...
  useEffect(() => {
    const isGenerating =
      isProcessing || pages.some((p) => p.status === "processing");
        ...
        <div className="bg-white p-12 rounded-[3rem] shadow-2xl max-w-md w-full text-center space-y-8 border border-indigo-50">
          ...
          <div className="space-y-3">
            ...
            <p className="text-slate-500 font-medium text-lg">
              Create beautifully illustrated children's books in minutes — no
          ...
          <p className="text-slate-400 text-xs font-medium">
            By signing in you agree to our terms of service. No credit card
            required to start.
          </p>
        </div>
      </div>
  ...
  const handleExportProjectFile = () => {
    ...
    const blob = new window.Blob([JSON.stringify(project, null, 2)], {
  ...
  const handleImportProjectFile = async (
    ...
    if (file.name.toLowerCase().endsWith(".pdf")) {
      ...
      try {
        const { extractProjectFromPDF, extractImagesFromPDF } =
          await import("./utils/pdfExtract");
        setIsProcessing(true);
  ...
  const handleDownloadLayers = async (page: BookPage, index: number) => {
    ...
    page.layers.forEach((layer, i) => {
      ...
      zip.file(`${i + 1}_${layer.name.replace(/\s+/g, "_")}.png`, base64Data, {
        base64: true,
      });
    });
  ...
  const handleAddMorePages = async () => {
    ...
    try {
      const result = await planStoryScenes(
        extraScript,
      ...
      if (result.characterIdentities && result.characterIdentities.length > 0) {
        const newChars = result.characterIdentities
          .filter(
            (ci) =>
              !settings.characterReferences.some(
                (cr) =>
                  cr.name.trim().toLowerCase() === ci.name.trim().toLowerCase(),
              ),
          )
          .map((ci) => ({
  ...
  const handlePlanStory = async () => {
    ...
    try {
      ...
      if (
        result.globalInstructions &&
        !settings.masterBible.includes(result.globalInstructions)
      ) {
        setSettings((prev) => ({
          ...prev,
      ...
      setPages(
        result.pages.map((p) => ({
          ...
          assignments: p.mappedCharacterNames.map((name) => ({
            refId: name,
            description: "",
          })),
          isSpread: p.isSpread,
      ...
      showToast("Script analysis failed.");
    } finally {
      setIsParsing(false);
    }
  };
  ...
  const handlePlanActivities = async () => {
    ...
    try {
      ...
      showToast("Activity analysis failed.");
    } finally {
      setIsParsing(false);
    }
  };

  const handleRestyleUpload = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    ...
    for (const f of files) {
      if (f.type === "application/pdf") {
        ...
        if (currentStep === "landing" || currentStep === "upload") {
          setCurrentStep(
            settings.mode === "retarget" ? "generate" : "restyle-editor",
          );
        }
  ...
  const handleAISelectFont = async () => {
    ...
    try {
      const sampleText =
        pages.find((p) => p.originalText)?.originalText ||
        "A brave little adventurer stepped carefully through the magical forest, wondering what surprises were hiding behind the giant glowing mushrooms.";
      const fontData = await selectStoryFont(
        settings.masterBible || projectContext || settings.targetStyle,
        sampleText,
      );
      setSettings((prev) => ({
        ...prev,
    ...
    reader.onload = async () => {
      ...
      img.onload = async () => {
        ...
        try {
          ...
          showToast(
            "Failed to analyze style: " + (error.message || "Unknown error"),
          );
        } finally {
          setIsAnalyzingStyle(false);
        }
  ...
  const renderScene = async (pageId: string) => {
    ...
    setPages((curr) =>
      curr.map((p) => (p.id === pageId ? { ...p, status: "processing" } : p)),
    );
    setIsProcessing(true);
    try {
      const p = pages.find((pg) => pg.id === pageId)!;
      const spreadInstruction = p.isSpread
        ? " IMPORTANT: This is a TWO-PAGE SPREAD illustration. DO NOT place character faces, text, or focal points directly on the vertical center line (the book's gutter) as they will be folded."
        : "";
      const basePrompt =
        p.overrideStylePrompt ||
        (p.originalText ? `SCENE SCRIPT: "${p.originalText}"` : "");
      ...
      const finalAspectRatio = p.isSpread
        ? getSpreadRatio(targetAspectRatio)
        : targetAspectRatio;
      ...
      if (p.environmentRefId) {
        ...
        if (refPage)
          envRefBase64 = refPage.processedImage || refPage.originalImage;
      }
      ...
      if (settings.layeredMode && !p.originalImage) {
        ...
        const others = pages
          .filter((pg) => pg.id !== pageId && pg.processedImage)
          .slice(0, 3)
          .map((pg) => ({
            base64: pg.processedImage!,
            index: pages.indexOf(pg) + 1,
          }));
        result = await refineIllustration(
          p.originalImage,
          narrativeContext,
          others,
          p.isSpread,
        ...
        if (envRefBase64 && p.borrowConfig) {
      ...
      setPages((curr) =>
        curr.map((pg) => (pg.id === pageId ? { ...pg, status: "error" } : pg)),
      );
    } finally {
      setIsProcessing(false);
    }
  ...
  const handleApplyAdvancedFix = async () => {
    ...
    setPages((curr) =>
      curr.map((p) =>
        p.id === activeFixId ? { ...p, status: "processing" } : p,
      ),
    );
    setIsProcessing(true);
    ...
    try {
      ...
      const selectedRefs = pages
        .filter(
          (pg) =>
            selectedRefIds.has(pg.id) &&
            (pg.processedImage || pg.originalImage),
        )
        .map((pg) => ({
      ...
      if (fixMode === "outpaint") {
        ...
        await new Promise<void>((resolve, reject) => {
          img.onload = () => resolve();
          img.onerror = () =>
            reject(new Error("Failed to load image for outpaint"));
        });
      ...
      if (fixMode === "separate-layers") {
        const layeredRes = await separateIllustrationIntoLayers(
          finalTargetImg,
          finalPrompt,
          selectedRefs,
          p.isSpread,
        ...
        const layeredRes = await refineLayeredIllustration(
          finalTargetImg,
          finalPrompt,
          selectedRefs,
          fixMode === "outpaint" ? true : p.isSpread,
          targetResolution,
          settings.masterBible,
          projectContext,
          settings.characterReferences,
      ...
      setPages((curr) =>
        curr.map((pg) =>
          pg.id === targetId ? { ...pg, status: "error" } : pg,
        ),
      );
    } finally {
      setIsProcessing(false);
  ...
  const handleRetargetSourceUpload = (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = e.target.files?.[0];
    if (!file) return;
  ...
  const handleApplyRetargeting = async () => {
    ...
    try {
      ...
      setPages((curr) =>
        curr.map((pg) =>
          pg.id === targetId
            ? { ...pg, status: "completed", processedImage: res }
            : pg,
        ),
      );
  ...
  const HotspotOverlay: React.FC<{
    ...
    return (
      <div
        ref={containerRef}
        className="relative cursor-crosshair group overflow-hidden rounded-[3rem] border-4 border-white shadow-2xl"
        onClick={handleClick}
      >
        ...
        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none flex items-center justify-center">
          <p className="text-white font-black uppercase tracking-widest text-xs bg-black/40 px-4 py-2 rounded-full">
    ...
    switch (currentStep) {
            ...
            <div className="max-w-5xl mx-auto py-16 px-8 space-y-16 animate-in fade-in duration-700">
              ...
              <div className="text-center space-y-6">
                ...
                <p className="text-slate-500 text-xl max-w-xl mx-auto font-medium leading-relaxed">
                  Write a story, pick an art style, and get a print-ready
                  illustrated book in minutes. No design skills needed.
                </p>

                {/* Credits status */}
                {userProfile && (
                  <div
                    className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-sm border ${
              ...
              {savedProjects.length === 0 && (
                <div className="text-center py-8 text-slate-300 space-y-4">
                  ...
                  <p className="text-slate-400 font-medium">
                    Create your first book above to get started!
                  </p>
                </div>
              )}
                      ...
                      >
                        ...
                        <div className="aspect-square bg-slate-100 overflow-hidden relative">
                          {renderProjectThumbnails(
                            proj,
                            "w-full h-full flex flex-wrap bg-slate-200 group-hover:scale-105 transition-transform origin-center",
                          )}
                        </div>
                        <div className="p-4">
                  ...
                  {[
                    {
                      ...
                      desc: "A few sentences is all you need",
                    },
                    {
                      step: "2",
                      icon: "🎨",
                      title: "Pick a style",
                      desc: "Choose from 4 beautiful art styles",
                    },
                    {
                      step: "3",
                      icon: "✨",
                      title: "AI illustrates",
                      desc: "Every page drawn automatically",
                    },
                    {
                      step: "4",
                      icon: "📖",
                      title: "Download & print",
                      desc: "Print-ready PDF in seconds",
                    },
                  ].map((item) => (
          ...
          <div className="max-w-6xl mx-auto py-24 px-8 space-y-24 animate-in fade-in duration-700">
            <div className="text-center space-y-4">
              ...
              <h2 className="text-8xl font-black text-slate-900 tracking-tighter">
                Series <span className="text-indigo-600">Master</span>
              </h2>
              <p className="text-slate-500 text-2xl max-w-2xl mx-auto font-medium">
                Professional Children's Book Production & Consistency Lab.
              </p>
              <button
                onClick={handleToggleUserMode}
                className="text-slate-400 font-bold text-sm hover:text-slate-600 underline transition-colors"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {recentProject && (
                <button
                  disabled={isProcessing}
                  onClick={() => handleLoadProject(recentProject)}
                  className="group p-10 bg-indigo-50 border-2 border-indigo-200 rounded-[4rem] text-left hover:border-indigo-600 hover:shadow-2xl transition-all relative overflow-hidden md:col-span-2 lg:col-span-3 flex items-center gap-8 disabled:opacity-50"
                >
                  <div className="w-24 h-24 bg-indigo-600 rounded-3xl flex items-center justify-center text-white shrink-0 group-hover:scale-110 transition-transform overflow-hidden shadow-inner">
                    {loadingProjectId === recentProject.id ? (
                      ...
                      renderProjectThumbnails(
                        recentProject,
                        "w-full h-full flex flex-wrap bg-slate-200",
                      )
                    )}
                  </div>
                  <div>
                    <h4 className="text-3xl font-black mb-2 text-slate-900">
                      {loadingProjectId === recentProject.id
                        ? "Loading Project..."
                        : `Resume Project: ${recentProject.name}`}
                    </h4>
                    <p className="text-indigo-600 font-medium text-lg">
                      Pick up right where you left off. All your generated pages
              ...
              <button
                onClick={() => {
                  setSettings({ ...settings, mode: "create" });
                  setCurrentStep("script");
                }}
                className="group p-10 bg-white border-2 border-slate-100 rounded-[4rem] text-left hover:border-indigo-600 hover:shadow-2xl transition-all relative overflow-hidden"
              >
                ...
                <h4 className="text-2xl font-black mb-3 text-slate-900">
                  Script Storyboarder
                </h4>
                <p className="text-slate-400 font-medium">
                  Auto-generate full series frames from your narrative script.
                </p>
              </button>
              <button
                onClick={() => {
                  setSettings({ ...settings, mode: "activity-builder" });
                  setCurrentStep("activity-builder");
                }}
                className="group p-10 bg-white border-2 border-indigo-100 rounded-[4rem] text-left hover:border-indigo-600 hover:shadow-2xl transition-all relative overflow-hidden"
              >
                <div className="w-16 h-16 bg-indigo-50 rounded-3xl flex items-center justify-center mb-8 text-indigo-600 group-hover:scale-110 transition-transform">
                  <Grid size={32} />
                </div>
                ...
                <p className="text-slate-400 font-medium">
                  Flashcards, spreads, and dictionaries with strict logic and
                  layout.
                </p>
              </button>
              <button
                onClick={() => {
                  setSettings({ ...settings, mode: "restyle" });
                  setCurrentStep("upload");
                }}
                className="group p-10 bg-white border-2 border-slate-100 rounded-[4rem] text-left hover:border-indigo-600 hover:shadow-2xl transition-all relative overflow-hidden"
              >
                <div className="w-16 h-16 bg-slate-50 rounded-3xl flex items-center justify-center mb-8 text-slate-400 group-hover:scale-110 transition-transform">
                  <Palette size={32} />
                </div>
                <h4 className="text-2xl font-black mb-3 text-slate-900">
                  Advanced Fixer
                ...
                <p className="text-slate-400 font-medium">
                  Outpaint, restyle, and fix details while referencing other
                  images.
                </p>
              </button>
              <button
                onClick={() => {
                  setSettings({ ...settings, mode: "retarget" });
                  setCurrentStep("upload");
                }}
                className="group p-10 bg-indigo-600 border-2 border-indigo-600 rounded-[4rem] text-left hover:shadow-2xl transition-all relative overflow-hidden"
              >
                <div className="w-16 h-16 bg-white/20 rounded-3xl flex items-center justify-center mb-8 text-white group-hover:scale-110 transition-transform">
                  <UserCheck size={32} />
                </div>
                <h4 className="text-2xl font-black mb-3 text-white">
                  Character Identity Lab
                </h4>
                <p className="text-indigo-100 font-medium mb-6">
                  Map faces and outfits from any reference photo using numerical
                  hotspots.
                </p>
                <div className="inline-flex items-center gap-2 px-6 py-3 bg-white text-indigo-600 rounded-2xl font-black text-xs uppercase tracking-widest">
                  Start Retargeting <ArrowRight size={16} />
                </div>
              </button>
              <button
                onClick={() => {
                ...
                className="group p-10 bg-white border-2 border-slate-100 rounded-[4rem] text-left hover:border-emerald-600 hover:shadow-2xl transition-all"
              >
                ...
                <p className="text-slate-400 font-medium">
                  Upscale and enhance your final frames for print quality.
                </p>
              </button>
              <button
                onClick={() => {
                  setSettings({ ...settings, mode: "cover-designer" });
                  setCurrentStep("cover-master");
                }}
                className="group p-10 bg-white border-2 border-slate-100 rounded-[4rem] text-left hover:border-amber-600 hover:shadow-2xl transition-all"
              >
                <div className="w-16 h-16 bg-amber-50 rounded-3xl flex items-center justify-center mb-8 text-amber-600 group-hover:scale-110 transition-transform">
                  <BookMarked size={32} />
                </div>
                <h4 className="text-2xl font-black mb-3 text-slate-900">
                  Cover Designer
                ...
                <p className="text-slate-400 font-medium">
                  Synthesize marketing context into a professional cover
                  independently.
                </p>
              </button>
              <button
                onClick={() => setCurrentStep("production-layout")}
                className="group p-10 bg-white border-2 border-indigo-100 rounded-[4rem] text-left hover:border-indigo-600 hover:shadow-2xl transition-all relative overflow-hidden"
              >
                <div className="w-16 h-16 bg-indigo-50 rounded-3xl flex items-center justify-center mb-8 text-indigo-600 group-hover:scale-110 transition-transform">
                  <Ruler size={32} />
                ...
                <p className="text-slate-400 font-medium">
                  Automate Bleed and Gutter for KDP and Lulu publishing.
                </p>
              </button>
              <button
                onClick={() => setCurrentStep("niche-research")}
                className="group p-10 bg-white border-2 border-rose-100 rounded-[4rem] text-left hover:border-rose-600 hover:shadow-2xl transition-all relative overflow-hidden"
              >
                <div className="w-16 h-16 bg-rose-50 rounded-3xl flex items-center justify-center mb-8 text-rose-600 group-hover:scale-110 transition-transform">
                  <TrendingUp size={32} />
                </div>
                <h4 className="text-2xl font-black mb-3 text-slate-900">
                  Niche Research
                ...
                <p className="text-slate-400 font-medium">
                  Analyze market demand, competition, and profitability for book
                  ideas.
                </p>
              </button>
              <button
                onClick={() => {
                  setSettings({ ...settings, mode: "kdp-fixer" });
                  setCurrentStep("kdp-fixer");
                }}
                className="group p-10 bg-white border-2 border-emerald-100 rounded-[4rem] text-left hover:border-emerald-600 hover:shadow-2xl transition-all relative overflow-hidden"
              >
                <div className="w-16 h-16 bg-emerald-50 rounded-3xl flex items-center justify-center mb-8 text-emerald-600 group-hover:scale-110 transition-transform">
                  <FileText size={32} />
                </div>
                ...
                <p className="text-slate-400 font-medium">
                  Upload a rejected PDF and KDP notes to automatically fix bleed
                  and margins.
                </p>
              </button>
            </div>
          ...
          <div className="max-w-5xl mx-auto py-20 px-8 space-y-12 animate-in slide-in-from-bottom duration-500">
            <div className="text-center space-y-4">
              ...
              <p className="text-slate-500 text-xl font-medium">
                Discover high-demand, low-competition book ideas.
              </p>
            </div>
            ...
            <div className="flex justify-center">
              <button
                ...
                className="py-6 px-12 bg-slate-100 text-slate-500 rounded-[2rem] font-black text-xl hover:bg-slate-200 transition-all"
              >
                BACK TO DASHBOARD
              </button>
            </div>
        ...
        return (
          <div className="max-w-5xl mx-auto py-20 px-8 space-y-12 animate-in slide-in-from-bottom duration-500">
            <div className="text-center space-y-4">
              ...
              <p className="text-slate-500 text-xl font-medium">
                Paste your high-logic Activity spreads here (Flashcards, Grids,
                Dictionaries).
              </p>
            </div>
            <textarea
            ...
            <div className="flex gap-8">
              <button
                onClick={() => setCurrentStep("landing")}
                className="flex-1 py-8 bg-slate-100 text-slate-500 rounded-[2.5rem] font-black text-2xl hover:bg-slate-200 transition-all"
              >
                CANCEL
              </button>
              <button
                disabled={isParsing || !activityScript}
                onClick={handlePlanActivities}
                className="flex-[2] py-8 bg-indigo-600 text-white rounded-[2.5rem] font-black text-3xl shadow-2xl flex items-center justify-center gap-6 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50"
              >
                {isParsing ? (
            ...
            <div className="flex flex-col lg:flex-row gap-16 items-start">
              <div className="flex-1 space-y-10 sticky top-36">
                <div className="bg-white border-2 border-slate-100 rounded-[4rem] p-12 shadow-2xl space-y-10">
                  {/* QUICK PRESETS */}
                  <div className="space-y-3">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-600 px-4">
                      Quick Presets
                    </h3>
                    <div className="grid grid-cols-2 gap-2 px-2">
                      ...
                      >
                        <div className="font-black text-xs text-slate-800 group-hover:text-indigo-600">
                          KDP Standard Square
                        </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center px-4">
                      ...
                      <button
                        onClick={() => setShowBibleEditor(true)}
                    ...
                    <div className="flex justify-between items-center px-4">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="text-[10px] font-black text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-lg hover:bg-indigo-100 transition-colors shadow-sm"
                        >
                          Soft Painterly
                        </button>
                        <button
                          onClick={() => {
                            setSettings({
                              ...settings,
                              targetStyle:
                                "Flat vector illustration, SVG style, solid colors, clean crisp edges, no gradients, no shading, minimalist, 2D flat design, perfect for auto-tracing",
                            });
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-4 gap-2 px-2">
                      {(["1:1", "4:3", "16:9", "9:16"] as const).map(
                        (ratio) => (
                          <button
                            key={ratio}
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {(["1K", "2K", "4K"] as const).map((res) => (
                        <button
                          ...
                          className={`py-4 rounded-[1.5rem] border-2 font-black text-lg transition-all ${targetResolution === res ? "border-indigo-600 bg-indigo-50 text-indigo-600 shadow-lg" : "border-slate-50 opacity-50"}`}
                        >
                          {res}
                        </button>
                      ))}
                    ...
                    <div className="flex justify-between items-center px-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-slate-400">
                          {settings.embedTextInImage ? "ON" : "OFF"}
                        </span>
                        <button
                          onClick={() =>
                            setSettings({
                              ...settings,
                              embedTextInImage: !settings.embedTextInImage,
                            })
                          }
                          className={`w-12 h-6 rounded-full transition-all relative ${settings.embedTextInImage ? "bg-indigo-600" : "bg-slate-200"}`}
                        >
                          <div
                            className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.embedTextInImage ? "left-7" : "left-1"}`}
                          />
                        </button>
                      </div>
                    </div>
                    <p className="text-[9px] text-slate-400 px-4 font-medium italic">
                      If ON, AI will attempt to render the scene text directly
                      into the illustration while respecting safe margins.
                    </p>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center px-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-slate-400">
                          {settings.layeredMode ? "ON" : "OFF"}
                        </span>
                        <button
                          onClick={() =>
                            setSettings({
                              ...settings,
                              layeredMode: !settings.layeredMode,
                            })
                          }
                          className={`w-12 h-6 rounded-full transition-all relative ${settings.layeredMode ? "bg-indigo-600" : "bg-slate-200"}`}
                        >
                          <div
                            className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.layeredMode ? "left-7" : "left-1"}`}
                          />
                        </button>
                      </div>
                    </div>
                    <p className="text-[9px] text-slate-400 px-4 font-medium italic">
                      If ON, AI generates separate layers for BG, Characters,
                      and Text for professional compositing.
                    </p>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center px-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <div
                            className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.overlayText ? "left-7" : "left-1"}`}
                          />
                        </button>
                      </div>
                    </div>
                    <p className="text-[9px] text-slate-400 px-4 font-medium italic">
                      If ON, the original text will be overlaid on the generated
                      PDF pages.
                    </p>
                    {settings.overlayText && (
                      <div className="px-4 mt-2 space-y-4">
                        <button
                          onClick={handleAISelectFont}
                          disabled={isProcessing}
                          className="w-full flex items-center justify-center gap-2 bg-indigo-50 text-indigo-600 rounded-xl py-3 font-black text-xs uppercase tracking-widest hover:bg-indigo-100 transition-colors"
                        >
                          {isProcessing ? (
                        ...
                        <div>
                          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 block mb-2">
                            Story Type
                          </label>
                          <select
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setSettings({
                                ...settings,
                                spreadTextSide: e.target.value as any,
                              })
                            }
                            className="w-full bg-slate-100 border-none rounded-xl text-sm font-bold text-slate-700 p-3"
                          >
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setSettings({
                                  ...settings,
                                  overlayTextColor: e.target.value,
                                })
                              }
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setSettings({
                                  ...settings,
                                  overlayTextPosition: e.target.value as any,
                                })
                              }
                              className="w-full bg-slate-100 border-none rounded-xl text-sm font-bold text-slate-700 p-3"
                            >
                        ...
                        <div>
                          ...
                          <select
                            value={
                              settings.overlayTextBackground || "transparent"
                            }
                            onChange={(e) =>
                              setSettings({
                                ...settings,
                                overlayTextBackground: e.target.value as any,
                              })
                            }
                            className="w-full bg-slate-100 border-none rounded-xl text-sm font-bold text-slate-700 p-3"
                          >
                            ...
                            <option value="semi-transparent-white">
                              Semi-Transparent White
                            </option>
                            <option value="semi-transparent-black">
                        ...
                        <label className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl cursor-pointer hover:bg-slate-100 transition-colors">
                          <input
                            ...
                            onChange={(e) =>
                              setSettings({
                                ...settings,
                                overlayTextShadow: e.target.checked,
                              })
                            }
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center px-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-slate-400">
                          {settings.showSafeGuides ? "ON" : "OFF"}
                        </span>
                        <button
                          onClick={() =>
                            setSettings({
                              ...settings,
                              showSafeGuides: !settings.showSafeGuides,
                            })
                          }
                          className={`w-12 h-6 rounded-full transition-all relative ${settings.showSafeGuides ? "bg-indigo-600" : "bg-slate-200"}`}
                        >
                          <div
                            className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.showSafeGuides ? "left-7" : "left-1"}`}
                          />
                        </button>
                      </div>
                    </div>
                    <p className="text-[9px] text-slate-400 px-4 font-medium italic">
                      Show bleed, gutter, and safe text margins for print-ready
                      layouts.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-8 bg-indigo-600 text-white rounded-[3rem] font-black text-2xl shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-6"
                  >
                    {isProcessing ? (
              ...
              <div className="w-full lg:w-2/5 grid grid-cols-1 gap-12">
                {pages.map((p, idx) => (
                  <div
                    key={p.id}
                    className="bg-white p-8 rounded-[4rem] border-2 border-slate-100 shadow-xl space-y-6 relative overflow-hidden group"
                  >
                    <div className="absolute top-8 left-8 z-10 w-14 h-14 bg-slate-900 text-white rounded-2xl flex items-center justify-center font-black text-2xl shadow-2xl">
                      #{idx + 1}
                    </div>
                    <div className="aspect-[4/3] bg-slate-100 rounded-[3rem] overflow-hidden shadow-inner border-8 border-white relative">
                      {(p.processedImage || p.originalImage) && (
                        <img
                          src={p.processedImage || p.originalImage}
                          className="w-full h-full object-cover"
                        />
                      )}
                      <SpreadGuide
                        isSpread={p.isSpread}
                        show={settings.showSafeGuides}
                        format={settings.exportFormat}
                        pageCount={settings.estimatedPageCount}
                      />
                      {settings.overlayText &&
                        p.originalText &&
                        p.textPositionOverride !== "hidden" && (
                          <div
                            className={`absolute inset-0 flex pointer-events-none p-[5%]
                          ${
                            (p.textPositionOverride ||
                              settings.overlayTextPosition) === "top"
                              ? "items-start"
                              : (p.textPositionOverride ||
                                    settings.overlayTextPosition) === "center"
                                ? "items-center"
                                : "items-end"
                          } justify-center`}
                          >
                            {(!p.isSpread ||
                              settings.spreadTextSide === "both") && (
                              <div
                                className={`${p.isSpread ? "w-1/2 flex justify-center" : "w-full flex justify-center"}`}
                              >
                                <div
                                  className={`
                                ...
                                >
                                  <p
                                    ...
                                    style={{
                                      ...
                                      color:
                                        settings.overlayTextColor || "#000000",
                                      textShadow:
                                        settings.overlayTextShadow !== false
                                          ? "0px 2px 4px rgba(0,0,0,0.5)"
                                          : "none",
                                    }}
                                  >
                                    {(() => {
                                      const parts = p.originalText
                                        .split("||")
                                        .map((t) => t.trim())
                                        .filter(Boolean);
                                      ...
                                      const fullText =
                                        parts.join("\n\n") || p.originalText;
                                      return settings.spreadTextSide === "both"
                                        ? leftHalfText
                                        : fullText;
                                    })()}
                                  </p>
                            ...
                            {p.isSpread &&
                              (settings.spreadTextSide === "right" ||
                                !settings.spreadTextSide) && (
                    ...
                    {p.originalText && (
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest px-4 italic leading-relaxed">
                        "{p.originalText}"
                      </p>
                    )}
                    ...
                    <div className="px-4 space-y-4">
                      {settings.overlayText && (
                        <div className="bg-slate-50 p-4 rounded-2xl flex flex-col gap-3">
                          <div className="flex items-center justify-between">
                            <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-500 leading-tight">
                              Position Override
                            </h4>
                            <div className="flex bg-slate-200 rounded-lg p-1">
                              {(
                                ["top", "center", "bottom", "hidden"] as const
                              ).map((pos) => (
                                <button
                                  key={pos}
                                  onClick={() =>
                                  ...
                                  className={`px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-md transition-all ${
                                    (p.textPositionOverride ||
                                      settings.overlayTextPosition) === pos
                                    ...
                                    setPages((curr) =>
                                      curr.map((pg) =>
                                        pg.id === p.id
                                          ? {
                                              ...pg,
                                              textPositionOverride: undefined,
                                            }
                                          : pg,
                                      ),
                                    )
                                ...
                                <button
                                  ...
                                  className={`px-2 py-1 text-[10px] font-black uppercase tracking-widest rounded-md transition-all ${
                                    (p.textBackgroundOverride ||
                                      settings.overlayTextBackground) === bg
                                      ? "bg-white shadow text-indigo-600"
                                      : "text-slate-500 hover:bg-slate-300"
                                  }`}
                                >
                                    ...
                                    setPages((curr) =>
                                      curr.map((pg) =>
                                        pg.id === p.id
                                          ? {
                                              ...pg,
                                              textBackgroundOverride: undefined,
                                            }
                                          : pg,
                                      ),
                                    )
                              ...
                              <button
                                onClick={async () => {
                                  try {
                                    ...
                                    const layout = await analyzeTextLayout(
                                      p.processedImage || p.originalImage!,
                                      p.originalText || "Hello",
                                    );
                                    setPages((curr) =>
                                      curr.map((pg) =>
                                        pg.id === p.id
                                          ? {
                                              ...
                                              textPositionOverride:
                                                layout.textPositionOverride,
                                              textBackgroundOverride:
                                                layout.textBackgroundOverride,
                                              originalText:
                                                layout.suggestedText ||
                                                pg.originalText,
                                            }
                                          : pg,
                                      ),
                      ...
                      <div className="flex flex-wrap gap-2">
                        {settings.characterReferences.map((char) => {
                          const isAssigned = p.assignments?.some(
                            (a) => a.refId === char.name,
                          );
                          return (
                            <button
                    ...
                    {p.layers && p.layers.length > 0 && (
                      <LayerManager
                        layers={p.layers}
                        onToggle={(layerId) => {
                          const newLayers = p.layers!.map((l) =>
                            l.id === layerId
                          ...
                          const visibleLayers = newLayers.filter(
                            (l) => l.isVisible,
                          );
                          let loaded = 0;
      ...
      case "script":
        return (
          <div className="max-w-4xl mx-auto py-20 px-8 space-y-12 animate-in slide-in-from-bottom duration-500">
            <div className="text-center space-y-4">
              <h2 className="text-6xl font-black">Narrative Analysis</h2>
              <p className="text-slate-500 text-xl font-medium">
                Parse your script into production scenes.
              </p>
            </div>
            <textarea
              className="w-full h-[500px] bg-white border-2 border-slate-100 rounded-[3rem] p-16 text-2xl font-medium outline-none shadow-inner leading-relaxed"
            ...
            <div className="flex items-center gap-4 px-4">
              <button
                onClick={() =>
                  setEnableActivityDesigner(!enableActivityDesigner)
                }
                className={`w-14 h-8 rounded-full transition-colors relative ${enableActivityDesigner ? "bg-indigo-600" : "bg-slate-200"}`}
              >
                <div
                  className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform ${enableActivityDesigner ? "translate-x-6" : "translate-x-0"}`}
                />
              </button>
              <div>
                <h4 className="font-black text-slate-900">
                  Enable Activity Designer
                </h4>
                <p className="text-sm text-slate-500 font-medium">
                  Extract global instructions and detailed prompts for activity
                  pages.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-8">
              <button
                onClick={() => setCurrentStep("landing")}
                className="flex-1 py-8 bg-slate-100 text-slate-500 rounded-[2.5rem] font-black text-2xl"
              >
                CANCEL
              </button>
              <button
          ...
          <div className="max-w-7xl mx-auto py-20 px-8 space-y-20 pb-56">
            ...
            <div className="space-y-10">
              <div className="text-center space-y-4">
                ...
                <p className="text-slate-500 text-xl font-medium">
                  Choose a predefined style or analyze your own image to lock
                  the series look.
                </p>
              </div>
                ...
                {PREDEFINED_STYLES.map((style) => (
                  <div
                    ...
                    onClick={() =>
                      setSettings((s) => ({
                        ...s,
                        masterBible: `STYLE LOCK: ${style.prompt}\n\n${s.masterBible}`,
                      }))
                    }
                  ...
                  >
                    <img
                      ...
                      onClick={() =>
                        setSettings((s) => ({
                          ...s,
                          masterBible: `STYLE LOCK (From Uploaded Reference): ${style.prompt}\n\n${s.masterBible}`,
                          styleReference: style.image,
                        }))
                      }
                    />
              ...
              <div className="bg-white border-2 border-slate-100 rounded-[3rem] p-10 shadow-xl flex flex-col md:flex-row gap-8 items-center">
                <div className="flex-1 space-y-4">
                  <h3 className="text-2xl font-black text-slate-900">
                    Analyze Custom Style
                  </h3>
                  <p className="text-slate-500 font-medium">
                    Upload an illustration you love. Our AI will analyze the
                    medium, lighting, and mood to generate a perfect style lock
                    prompt.
                  </p>
                ...
                <div className="flex-[2] w-full">
                  <textarea
                    ...
                    value={settings.masterBible}
                    onChange={(e) =>
                      setSettings({ ...settings, masterBible: e.target.value })
                    }
            ...
            <div className="text-center space-y-4">
              <h2 className="text-6xl font-black">Consistency Lab</h2>
              <p className="text-slate-500 text-xl font-medium">
                Define your cast to lock identities across the series.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
              {settings.characterReferences.map((char) => (
                <div
                  key={char.id}
                  className="bg-white p-10 rounded-[4.5rem] border-2 border-slate-100 shadow-xl space-y-8 group transition-all"
                >
                  <div className="aspect-square bg-slate-50 rounded-[3rem] overflow-hidden relative flex items-center justify-center group">
                    ...
                    <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
                      <button
                        ...
                        className="p-5 bg-white rounded-2xl text-indigo-600 shadow-2xl"
                      >
                        <Upload size={24} />
                      </button>
                      <button
                        onClick={() =>
                          setSettings((s) => ({
                            ...s,
                            characterReferences: s.characterReferences.map(
                              (c) =>
                                c.id === char.id ? { ...c, images: [] } : c,
                            ),
                          }))
                        }
                        className="p-5 bg-white rounded-2xl text-red-500 shadow-2xl"
                      >
                        <Trash2 size={24} />
                      </button>
                    </div>
                        ...
                        identifyAndDesignCharacters(
                          ...
                          setSettings((s) => ({
                            ...
                            characterReferences: s.characterReferences.map(
                              (c) =>
                                c.id === char.id
                                  ? { ...c, images: res[0].images }
                                  : c,
                            ),
                          }));
                        })
                      }
                      className="w-full py-5 bg-indigo-50 text-indigo-600 rounded-[2rem] font-black text-sm uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all shadow-sm"
                    >
                      AI DESIGN SHEET
                    </button>
                  </div>
              ...
              <button
                ...
                className="border-4 border-dashed border-slate-100 rounded-[4.5rem] p-12 flex flex-col items-center justify-center gap-8 text-slate-300 hover:border-indigo-600 transition-all bg-white/50 group"
              >
                <Plus
                  size={80}
          ...
          <div className="max-w-7xl mx-auto py-16 px-8 space-y-20 pb-64">
              ...
              <div className="flex flex-col items-center justify-center gap-4">
                <div className="flex items-center gap-4">
                  <p className="text-slate-500 text-2xl font-medium">
                    Refining{" "}
                    {settings.mode === "activity-builder"
                      ? "Activity Spreads"
                      : "Series Frames"}{" "}
                    at {targetResolution}.
                  </p>
                  {settings.mode === "retarget" && (
                    <span className="px-4 py-1 bg-indigo-600 text-white text-xs font-black rounded-full animate-pulse">
                      RETARGETING MODE
                    </span>
                  )}
                </div>
                ...
                {isProcessing && (
                  <button
                    ...
                    className="mt-4 px-8 py-4 bg-red-500 text-white rounded-full font-black text-xl shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center gap-3"
                  >
                    <X size={24} /> STOP GENERATION
                  </button>
                )}
                  ...
                  <div className="flex items-center justify-center gap-4 mt-6">
                    <button
                      onClick={() => {
                        if (
                          confirm(
                            "Are you sure you want to regenerate all pages? This will overwrite existing outputs.",
                          )
                        ) {
                          setPages((curr) =>
                            curr.map((p) => ({
            ...
            <div className="bg-slate-50 border-4 border-slate-100 rounded-[4rem] p-12 space-y-6 flex flex-col items-center max-w-4xl mx-auto shadow-inner text-center">
              <h3 className="text-3xl font-black text-slate-800">
                Need to append more scenes?
              </h3>
              <p className="text-slate-500 font-bold text-lg">
                Paste additional script segments here, and we'll analyze and add
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
              {pages.map((p, idx) => (
                <div
                  key={p.id}
                  className="bg-white rounded-[6rem] border-4 border-slate-50 shadow-2xl overflow-hidden group transition-all"
                >
                  <div className="aspect-[16/9] bg-slate-100 relative group overflow-hidden">
                    {(p.processedImage || p.originalImage) && (
                      <img
                        src={p.processedImage || p.originalImage}
                        className={`w-full h-full object-cover transition-all duration-1000 ${p.status === "processing" ? "opacity-30 blur-2xl scale-110" : "opacity-100"}`}
                      />
                    )}
                    <SpreadGuide
                      isSpread={p.isSpread}
                    ...
                    {p.status === "error" && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center bg-red-50/90 backdrop-blur-sm p-8 text-center">
                        ...
                        <p className="text-red-400 text-xs font-medium mt-2">
                          Check your API key or try a simpler prompt.
                        </p>
                        <button
                    ...
                    <div className="absolute top-12 left-12 z-10 w-20 h-20 bg-slate-900 text-white rounded-[2rem] flex items-center justify-center font-black text-4xl shadow-2xl">
                      #{idx + 1}
                    </div>
                    <div className="absolute top-12 right-12 z-10 bg-emerald-500 text-white px-8 py-3 rounded-full font-black text-sm shadow-2xl">
                      {targetResolution} {p.isSpread ? "(SPREAD)" : "(SINGLE)"}
                    </div>
                    {p.status === "completed" && (
                      <button
                        onClick={() =>
                  ...
                  <div className="p-16 space-y-10">
                    <div className="space-y-4">
                      <h4 className="text-xs font-black uppercase tracking-[0.2em] text-indigo-600 flex items-center gap-3">
                        <Sparkles size={20} /> Image Generation Prompt
                      </h4>
                      <textarea
                        ...
                        onChange={(e) =>
                          setPages((curr) =>
                            curr.map((pg) =>
                              pg.id === p.id
                                ? { ...pg, overrideStylePrompt: e.target.value }
                                : pg,
                            ),
                          )
                        }
                    ...
                    <div className="space-y-4">
                      <h4 className="text-xs font-black uppercase tracking-[0.2em] text-indigo-600 flex items-center gap-3">
                        <PenTool size={20} /> Editable PDF Text Overlay
                      </h4>
                      <textarea
                        ...
                        onChange={(e) =>
                          setPages((curr) =>
                            curr.map((pg) =>
                              pg.id === p.id
                                ? { ...pg, originalText: e.target.value }
                                : pg,
                            ),
                          )
                        }
                    ...
                    <div className="space-y-4">
                      <h4 className="text-xs font-black uppercase tracking-[0.2em] text-indigo-600 flex items-center gap-3">
                        <ImageIcon size={20} /> Environment Reference (Visual
                        Link)
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        >
                          <option value="">
                            None (Auto-generate environment)
                          </option>
                          {pages.map((refPg, i) =>
                            refPg.id !== p.id ? (
                              <option key={refPg.id} value={refPg.id}>
                                Link to Scene #{i + 1}
                              </option>
                            ) : null,
                          )}
                        ...
                        {p.environmentRefId && (
                          <div className="flex flex-col gap-3 mt-4 md:mt-0 col-span-1 md:col-span-2 bg-indigo-50/50 p-6 rounded-3xl border border-indigo-100">
                            <p className="text-xs font-black uppercase tracking-widest text-indigo-800 mb-2">
                              What to borrow from this scene:
                            </p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {[
                                {
                                  ...
                                  desc: "Maintains the exact location, time of day, and weather shown in the referenced scene.",
                                },
                                {
                                  key: "characters",
                                  label: "Character Identities",
                                  desc: "Maintains the exact character body types and facial features from the reference.",
                                },
                                {
                                  key: "clothing",
                                  label: "Clothing & Outfits",
                                  desc: "Uses the identical outfits from the reference image instead of generating new clothes.",
                                },
                                {
                                  key: "poses",
                                  label: "Poses & Expressions",
                                  desc: "Copies the characters' actions and poses. Uncheck this if you want them to move, act differently, or change expressions based on the text prompt.",
                                },
                              ].map((opt) => {
                                const isChecked = !!(p.borrowConfig || {
                                  environment: true,
                                  characters: false,
                                  clothing: false,
                                  poses: false,
                                })[
                                  opt.key as keyof import("./types").BorrowConfig
                                ];
                                return (
                                  <label
                                    key={opt.key}
                                    className={`flex items-start gap-4 cursor-pointer group p-4 rounded-2xl border-2 transition-all ${isChecked ? "bg-white border-indigo-500 shadow-sm" : "bg-white/50 border-transparent hover:bg-white hover:border-slate-200"}`}
                                  >
                                    <div
                                      className={`w-6 h-6 rounded-lg flex items-center justify-center transition-all shrink-0 mt-0.5 ${isChecked ? "bg-indigo-600 text-white" : "bg-slate-200 text-transparent"}`}
                                    >
                                      <Check size={14} strokeWidth={4} />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                      <div
                                        className={`text-sm font-bold flex items-center justify-between ${isChecked ? "text-indigo-900" : "text-slate-600"}`}
                                      >
                                        <span>{opt.label}</span>
                                      ...
                                      <div className="text-xs font-medium text-slate-500/80 mt-1 truncate pr-4">
                                        {opt.desc}
                                      </div>
                                    </div>
                                    <input
                    ...
                    <div className="flex items-center justify-between gap-6 pt-10 border-t border-slate-50">
                      {settings.mode === "retarget" ? (
                        <>
                          <button
                            onClick={() => {
                              ...
                              if (p.retargeting?.sourceImage) {
                                setRetargetSourceImage(
                                  p.retargeting.sourceImage,
                                );
                              }
                              setPages((curr) =>
                                curr.map((pg) =>
                          ...
                          <button
                            ...
                            className="flex-1 py-7 bg-slate-100 text-slate-400 rounded-[2rem] font-black text-xs uppercase tracking-widest flex items-center justify-center hover:bg-slate-200 transition-all"
                          >
                            FIX
                          </button>
                        </>
                        ...
                        <>
                          <button
                            ...
                            className="flex-[2] py-7 bg-indigo-50 text-indigo-600 rounded-[2rem] font-black text-xl uppercase tracking-widest flex items-center justify-center gap-4 hover:bg-indigo-600 hover:text-white transition-all shadow-sm"
                          >
                      ...
                      <button
                        onClick={() => renderScene(p.id)}
                        className="p-7 bg-slate-100 text-slate-400 rounded-[2rem] hover:text-indigo-600 transition-all"
                        title="Regenerate Page"
                      >
                        <RefreshCw size={32} />
                      </button>
                      <label
                        className="p-7 bg-amber-50 text-amber-600 rounded-[2rem] hover:bg-amber-600 hover:text-white transition-all cursor-pointer flex flex-col items-center justify-center"
                        title="Restore Image from PDF"
                      >
                        <Upload size={32} />
                        <input
                          type="file"
                      ...
                      {p.processedImage && (
                        <button
                          onClick={() => {
                            const a = document.createElement("a");
                            a.href = p.processedImage!;
                            a.download = `page_${idx + 1}.png`;
                            a.click();
                          }}
                          className="p-7 bg-emerald-50 text-emerald-600 rounded-[2rem]"
                        >
                          <Download size={32} />
                        </button>
                      )}
                      {p.layers && p.layers.length > 0 && (
                        <button
                          onClick={() => handleDownloadLayers(p, idx)}
                          className="p-7 bg-indigo-50 text-indigo-600 rounded-[2rem] flex flex-col items-center gap-1"
                        >
                          <Layers size={32} />
                      ...
                      <button
                        ...
                        className="p-7 bg-rose-50 text-rose-400 rounded-[2rem] hover:bg-rose-500 hover:text-white transition-all"
                        title="Delete Page"
                      >
                        <Trash2 size={32} />
                      </button>
              ...
              <div className="fixed inset-0 z-[100] flex items-center justify-center p-12">
                ...
                <div className="bg-white w-full max-w-md rounded-[3rem] p-12 shadow-2xl relative z-10 space-y-8 animate-in zoom-in duration-300 text-center">
                  ...
                  <p className="text-slate-500 font-medium">
                    ...
                    remove this page from your project?
                  </p>
                  <div className="flex gap-4 pt-4">
                    ...
                    <button
                      onClick={() => {
                        setPages((curr) =>
                          curr.filter((pg) => pg.id !== confirmDeleteId),
                        );
                        setConfirmDeleteId(null);
                        showToast("Page deleted successfully.");
                      }}
                      className="flex-1 py-4 bg-rose-500 text-white rounded-2xl font-bold hover:bg-rose-600 shadow-lg shadow-rose-500/30 transition-all"
                    >
                      Delete
                    </button>
                  </div>
              ...
              <div className="fixed inset-0 z-[100] flex items-center justify-center p-12 overflow-y-auto">
                ...
                <div className="bg-white w-full max-w-6xl rounded-[5rem] p-20 shadow-2xl relative z-10 space-y-12 animate-in zoom-in duration-300">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-5xl font-black tracking-tighter">
                        Advanced Scene Fixer
                      </h3>
                      <p className="text-slate-400 font-bold text-lg mt-2 uppercase tracking-widest">
                        Inject style, clothing, or layout from other frames.
                      </p>
                    </div>
                    <button
                      onClick={() => setActiveFixId(null)}
                      className="p-4 bg-slate-100 rounded-full hover:bg-slate-200"
                    >
                      <X size={32} />
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                    <div className="space-y-10">
                      <div className="space-y-4">
                        <h4 className="text-xs font-black uppercase text-indigo-600 tracking-widest">
                          1. Technical Instruction
                        </h4>
                        <textarea
                          className="w-full h-48 bg-slate-50 border-none rounded-[2.5rem] p-10 text-xl font-bold outline-none shadow-inner leading-relaxed"
                          placeholder="E.g., 'Change her hair to red', 'Use the clothing from Frame 2'..."
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="flex flex-col gap-3">
                          <button
                            onClick={() => setFixMode("targeted")}
                            className={`w-full py-4 rounded-[2rem] font-black text-xl flex items-center justify-center gap-4 transition-all ${fixMode === "targeted" ? "bg-indigo-600 text-white shadow-2xl" : "bg-slate-100 text-slate-400 hover:bg-slate-200"}`}
                          >
                            <Sparkles size={24} /> TARGETED DETAIL FIX
                          </button>
                          <button
                            onClick={() => setFixMode("outpaint")}
                            className={`w-full py-4 rounded-[2rem] font-black text-xl flex items-center justify-center gap-4 transition-all ${fixMode === "outpaint" ? "bg-indigo-600 text-white shadow-2xl" : "bg-slate-100 text-slate-400 hover:bg-slate-200"}`}
                          >
                          ...
                          {fixMode === "outpaint" && (
                            <div className="bg-indigo-50 rounded-[2rem] p-6 space-y-4 mb-2 shadow-inner border border-indigo-100">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex gap-2">
                                  <button
                                    onClick={() => setOutpaintPos("left")}
                                    className={`px-4 py-2 rounded-xl text-sm font-bold transition-colors ${outpaintPos === "left" ? "bg-indigo-600 text-white shadow-md" : "bg-white text-indigo-600 hover:bg-indigo-100"}`}
                                  >
                                    Left
                                  </button>
                                  <button
                                    onClick={() => setOutpaintPos("center")}
                                    className={`px-4 py-2 rounded-xl text-sm font-bold transition-colors ${outpaintPos === "center" ? "bg-indigo-600 text-white shadow-md" : "bg-white text-indigo-600 hover:bg-indigo-100"}`}
                                  >
                                    Center
                                  </button>
                                  <button
                                    onClick={() => setOutpaintPos("right")}
                                    className={`px-4 py-2 rounded-xl text-sm font-bold transition-colors ${outpaintPos === "right" ? "bg-indigo-600 text-white shadow-md" : "bg-white text-indigo-600 hover:bg-indigo-100"}`}
                                  >
                                    Right
                                  </button>
                                </div>
                              ...
                              <div className="space-y-2">
                                ...
                                <input
                                  ...
                                  value={outpaintScale}
                                  onChange={(e) =>
                                    setOutpaintScale(parseFloat(e.target.value))
                                  }
                          ...
                          <button
                            onClick={() => setFixMode("separate-layers")}
                            className={`w-full py-4 rounded-[2rem] font-black text-xl flex items-center justify-center gap-4 transition-all ${fixMode === "separate-layers" ? "bg-indigo-600 text-white shadow-2xl" : "bg-slate-100 text-slate-400 hover:bg-slate-200"}`}
                          >
                        ...
                        <p className="text-sm text-slate-400 font-medium px-4 text-center italic">
                          {fixMode === "outpaint" &&
                            "Transform will expand the canvas ratio intelligently while keeping the core characters consistent."}
                          {fixMode === "targeted" &&
                            "Fix specific details, characters, or styles in the current frame."}
                          {fixMode === "separate-layers" &&
                            "Extracts the image into distinct layers (Background, Characters, Text Bubbles, Text) for professional compositing."}
                        </p>
                      </div>

                      <button
                        onClick={handleApplyAdvancedFix}
                        className="w-full py-10 bg-indigo-600 text-white rounded-[3rem] font-black text-3xl shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-6"
                      >
                    ...
                    <div className="space-y-8">
                      {fixMode === "outpaint" ? (
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-2 gap-6 h-[500px] overflow-y-auto pr-4 custom-scrollbar">
                            {pages.map((p, i) => (
                              <div
                                ...
                                className={`aspect-square rounded-[2rem] border-4 overflow-hidden relative cursor-pointer transition-all ${selectedRefIds.has(p.id) ? "border-indigo-600 scale-95 shadow-2xl" : "border-transparent opacity-60 hover:opacity-100"}`}
                              >
                                <div className="absolute top-4 left-4 z-10 w-10 h-10 bg-black/60 text-white rounded-xl flex items-center justify-center font-black text-lg">
                                  #{i + 1}
                                </div>
                                <img
                                  src={p.processedImage || p.originalImage}
                          ...
                          <p className="text-sm text-slate-400 font-bold uppercase text-center tracking-widest">
                            Select frames to inject visual likeness/clothing.
                          </p>
                        </>
                      )}
                    </div>
            ...
            <div className="fixed bottom-0 inset-x-0 bg-slate-900 text-white p-16 z-50 rounded-t-[8rem] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-12">
              <div className="flex items-center gap-10">
                <div className="w-28 h-28 bg-white/10 rounded-full flex items-center justify-center text-indigo-400 font-black text-5xl border border-white/5">
                  {stats.progress}%
                </div>
                <div>
                  ...
                  <p className="text-slate-400 font-bold uppercase tracking-widest text-lg">
                    Finalizing Master Series Assets
                  </p>
                </div>
              </div>
              <button
                onClick={() => setCurrentStep("production-layout")}
                className="bg-indigo-600 px-32 py-12 rounded-[4rem] font-black text-4xl shadow-2xl hover:scale-105 transition-all flex items-center gap-10 active:scale-95"
              >
                PREPARE FOR PRINT <Download size={56} />
              </button>
            </div>
          </div>
        );
          ...
          <div className="max-w-7xl mx-auto py-24 px-8 space-y-20 animate-in fade-in duration-500 pb-56">
            <div className="flex flex-col lg:flex-row gap-20 items-start">
              <div className="flex-1 space-y-12 sticky top-36">
                <div className="space-y-4">
                  <h2 className="text-7xl font-black">Interior Master</h2>
                  <p className="text-slate-500 text-2xl font-medium">
                    Standardized layout for professional KDP and Lulu
                    publishing.
                  </p>
                </div>
                <div className="bg-white border-2 border-slate-100 rounded-[5rem] p-16 shadow-2xl space-y-12">
                  <div className="grid grid-cols-2 gap-6 max-h-[50vh] overflow-y-auto p-4 -m-4">
                    {(
                      Object.keys(
                        PRINT_FORMATS,
                      ) as (keyof typeof PRINT_FORMATS)[]
                    ).map((key) => (
                      <button
                        key={key}
                        onClick={() =>
                          setSettings({ ...settings, exportFormat: key })
                        }
                        className={`p-8 rounded-[2rem] border-2 text-left transition-all ${settings.exportFormat === key ? "border-indigo-600 bg-indigo-50 shadow-xl" : "border-slate-100 opacity-60 hover:opacity-100 hover:border-slate-300"}`}
                      >
                        <div className="font-black text-xl text-slate-800">
                          {PRINT_FORMATS[key].name}
                        </div>
                  ...
                  <div className="space-y-6">
                    <button
                      ...
                      onClick={async () => {
                        setIsProcessing(true);
                        try {
                          const { generateCoreBookPDF } =
                            await import("./utils/corePdfGenerator");
                          await generateCoreBookPDF(
                            pages,
                            settings.exportFormat,
                            projectName,
                            settings.overlayText,
                            settings.estimatedPageCount,
                            settings.spreadExportMode,
                    ...
                    <button
                      ...
                      onClick={async () => {
                        setIsProcessing(true);
                        try {
                          await generateLayeredEditablePDF(
                            pages,
                            settings.exportFormat,
                            projectName,
                            settings.estimatedPageCount,
                          ...
                          showToast(
                            "Failed to generate editable PDF: " +
                              (err?.message ||
                                (typeof err === "string"
                                  ? err
                                  : "Unknown error")),
                    ...
                    <button
                      ...
                      onClick={async () => {
                        setIsProcessing(true);
                        try {
                          const { generateTextOnlyPDF } =
                            await import("./utils/textPdfGenerator");
                          ...
                          showToast(
                            "Failed to generate Text Only PDF: " +
                    ...
                    <button
                      onClick={() =>
                        exportProjectAssetsForCanva(pages, projectName)
                      }
                      className="w-full py-8 bg-indigo-600 text-white rounded-[3rem] font-black text-2xl shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-4"
                    >
                      <Layers size={32} /> DOWNLOAD ASSETS FOR CANVA / PHOTOSHOP
                    </button>
                    <button
                      onClick={() => setCurrentStep("cover-master")}
                      className="w-full py-8 bg-amber-50 text-amber-600 rounded-[3rem] font-black text-2xl shadow-sm hover:bg-amber-100 transition-all flex items-center justify-center gap-4"
                    >
                      GO TO COVER DESIGNER <ChevronRight size={32} />
                    </button>
                  </div>
                </div>
              </div>
        ...
        return (
          <div className="max-w-7xl mx-auto py-20 px-8 space-y-16 animate-in fade-in duration-500 pb-64">
            <div className="flex flex-col lg:flex-row gap-16 items-start">
              <div className="flex-1 space-y-12">
                <div className="space-y-4">
                  ...
                  <p className="text-slate-500 text-xl font-medium">
                    Design a high-end production cover using your series cast.
                  </p>
                </div>
                ...
                <div className="bg-white border-2 border-slate-100 rounded-[4rem] p-12 shadow-2xl space-y-8">
                  {settings.mode === "cover-designer" && (
                    <div className="space-y-6 pb-6 border-b-2 border-slate-100">
                      ...
                      <textarea
                        className="w-full h-24 bg-slate-50 border-none rounded-2xl p-6 text-sm font-medium outline-none resize-none shadow-inner focus:ring-2 ring-indigo-600 transition-all"
                        placeholder="E.g. Vintage watercolor, cinematic lighting..."
                        value={settings.targetStyle}
                        onChange={(e) =>
                          setSettings({
                            ...settings,
                      ...
                      <div className="flex gap-4">
                        <div className="flex-1 space-y-2">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setTargetAspectRatio(e.target.value as any)
                            }
                            className="w-full bg-slate-100 rounded-xl p-3 outline-none font-bold text-slate-700"
                          >
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setTargetResolution(e.target.value as any)
                            }
                            className="w-full bg-slate-100 rounded-xl p-3 outline-none font-bold text-slate-700"
                          >
                      ...
                      <div className="flex items-center justify-between bg-slate-50 p-4 rounded-xl">
                        <span className="text-xs font-bold text-slate-600">
                          Generate Title Typography Layer
                        </span>
                        <button
                          onClick={() =>
                            setSettings({
                          ...
                          className={`w-12 h-6 rounded-full transition-all relative ${settings.layeredMode ? "bg-indigo-600" : "bg-slate-300"}`}
                        >
                          <div
                            className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.layeredMode ? "left-7" : "left-1"}`}
                          />
                  ...
                  <div className="space-y-4">
                    <h3 className="text-xs font-black uppercase tracking-widest text-indigo-600">
                      Marketing Brief / Scene Description
                    </h3>
                    <textarea
                      className="w-full h-64 bg-slate-50 border-none rounded-3xl p-8 text-lg font-medium outline-none resize-none shadow-inner focus:ring-2 ring-indigo-600 transition-all"
                      placeholder="Describe the cover scene, mood, and composition..."
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (
                              result.characterIdentities &&
                              result.characterIdentities.length > 0
                            ) {
                              const newChars = result.characterIdentities
                                .filter(
                                  (ci) =>
                              ...
                              result.characterIdentities.forEach((ci) => {
                                const existing =
                                  settings.characterReferences.find(
                                    (cr) =>
                                      cr.name.trim().toLowerCase() ===
                                      ci.name.trim().toLowerCase(),
                                  );
                  ...
                  <div className="flex gap-4">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        setIsProcessing(true);
                        const selectedChars =
                          settings.characterReferences.filter((c) =>
                            selectedCoverCharIds.has(c.id),
                          );
                        ...
                        if (settings.layeredMode) {
                          generateLayeredCover(
                            ...
                            .catch((err) => {
                              console.error(err);
                              setToastMessage(
                                err.message || "Failed to generate cover",
                              );
                              setTimeout(() => setToastMessage(null), 3000);
                            })
                    ...
                    >
                      ...
                      RENDER PRODUCTION COVER
                    </button>
                    <label
                      className="flex-1 bg-slate-100 rounded-[2.5rem] flex flex-col items-center justify-center text-slate-400 hover:bg-indigo-600 hover:text-white transition-all cursor-pointer shadow-sm group font-black text-xs uppercase tracking-widest gap-2 py-4 px-2 text-center"
                      title="Restore Cover Image from PDF"
                    >
                      <Upload
                        size={28}
                        className="group-hover:scale-110 transition-transform"
              ...
              <div
                className={`w-full lg:w-2/5 bg-white rounded-[4.5rem] shadow-2xl overflow-hidden border-8 border-white relative group ${!coverImage ? "flex flex-col items-center justify-center" : ""} ${targetAspectRatio === "16:9" ? "aspect-video" : targetAspectRatio === "1:1" ? "aspect-square" : targetAspectRatio === "9:16" ? "aspect-[9/16]" : "aspect-[4/3]"}`}
              >
                {coverImage ? (
                  <div className="absolute inset-0 w-full h-full">
                    ...
                    {coverLayers && coverLayers.length > 0 && (
                      <div className="absolute bottom-0 inset-x-0 bg-white/90 backdrop-blur-xl p-6 border-t border-slate-100">
                        <LayerManager
                          layers={coverLayers}
                          onToggle={(layerId) => {
                            const newLayers = coverLayers.map((l) =>
                              l.id === layerId
                                ? { ...l, isVisible: !l.isVisible }
                                : l,
                            );
                    ...
                    <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-6">
                      <button
                        ...
                        className="p-6 bg-white rounded-3xl text-indigo-600 shadow-2xl hover:scale-110 transition-transform"
                        title="Download PNG"
                      >
                        <Download size={32} />
                      </button>
                      <button
                        ...
                        className="p-6 bg-emerald-500 rounded-3xl text-white shadow-2xl hover:scale-110 transition-transform"
                      ...
                      <button
                        ...
                        className="p-6 bg-white rounded-3xl text-red-500 shadow-2xl hover:scale-110 transition-transform"
                        title="Delete Cover"
                      >
                        <Trash2 size={32} />
                      </button>
                ...
                {isProcessing && (
                  <div className="absolute inset-0 bg-indigo-600/10 backdrop-blur-sm flex flex-col items-center justify-center gap-6">
                    <Loader2
                      ...
                      className="animate-spin text-indigo-600"
                    />
                    <p className="text-indigo-600 font-black uppercase tracking-widest animate-pulse">
                      Synthesizing Cover...
                    </p>
                  </div>
                )}
            ...
            <div className="flex justify-center mt-12">
              <button
                onClick={() => setCurrentStep("production-layout")}
                className="text-slate-400 font-bold hover:text-slate-600 underline text-xl"
              >
                Back to Interior Master
              </button>
            </div>
          </div>
        );
      ...
      case "direct-upscale":
        return (
          <div className="max-w-4xl mx-auto py-20 px-8 space-y-12 text-center">
            ...
            <div
              onClick={() => !isProcessing && restyleInputRef.current?.click()}
              className={`aspect-video bg-white border-4 border-dashed border-slate-200 rounded-[5rem] flex flex-col items-center justify-center transition-all group shadow-inner ${isProcessing ? "cursor-not-allowed" : "cursor-pointer hover:border-emerald-500"}`}
            >
              {isProcessing ? (
                <div className="flex flex-col items-center gap-6">
                  ...
                  />
                  <p className="text-emerald-500 font-black uppercase tracking-[0.2em] text-xl animate-pulse">
                    Loading Assets...
                  </p>
                </div>
              ) : (
                <>
                  <Upload
                    size={80}
                    className="text-slate-200 group-hover:text-emerald-500 mb-8"
                  />
                  <p className="text-slate-400 font-black uppercase tracking-[0.2em] text-xl group-hover:text-emerald-500">
                    Select Frames to Master
                  </p>
                </>
              )}
              <input
            ...
            <button
              disabled={isProcessing}
              onClick={() => setCurrentStep("landing")}
              className="text-slate-400 font-bold hover:text-slate-600 underline disabled:opacity-50"
            >
              Back to Main Suit
            </button>
          </div>
      ...
      case "retarget-editor":
        ...
        const updateHotspot = (
          ...
          const retargeting = retargetPage.retargeting || {
            sourceHotspots: [],
            targetHotspots: [],
            instruction: "",
          };
          ...
          const filtered = existing.filter(
            (h) => h.label !== activeHotspotLabel,
          );
          setRetargetData({
            [key]: [...filtered, { x, y, label: activeHotspotLabel }],
          });
        };
        ...
        return (
          <div className="max-w-7xl mx-auto py-16 px-8 space-y-12 animate-in fade-in duration-500 pb-64">
            <div className="flex justify-between items-end">
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  <div className="px-4 py-1 bg-indigo-600 text-white text-[10px] font-black rounded-full uppercase tracking-widest">
                    Character Identity Lab
                  </div>
                ...
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-3 bg-white border-2 border-slate-100 rounded-2xl p-2 shadow-sm">
                    <span className="text-[10px] font-black uppercase text-slate-400 px-3">
                      Active Character:
                    </span>
                    {[1, 2, 3, 4, 5].map((num) => (
                      <button
                  ...
                  <p className="text-slate-400 text-sm font-medium italic">
                    Select a number, then click on both images to pair them.
                  </p>
                </div>
              </div>
              <div className="flex gap-4">
                <button
                  ...
                  className="px-8 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black uppercase tracking-widest"
                >
                  Cancel
                </button>
                <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center px-4">
                  ...
                  <button
                    onClick={() => retargetSourceInputRef.current?.click()}
                    className="text-[10px] font-black text-indigo-600 underline"
                  >
                    Upload External
                  </button>
                  <input
                    type="file"
                    ref={retargetSourceInputRef}
                    className="hidden"
                    accept="image/*"
                    onChange={handleRetargetSourceUpload}
                  />
                </div>
                  ...
                  {retargetSourceImage ? (
                    <HotspotOverlay
                      ...
                      onRemoveHotspot={(label) =>
                        setRetargetData({
                          sourceHotspots:
                            retargetPage.retargeting!.sourceHotspots.filter(
                              (h) => h.label !== label,
                            ),
                        })
                      }
                    ...
                    <div className="aspect-square bg-slate-100 rounded-[3rem] flex items-center justify-center text-slate-300 font-black italic border-4 border-dashed border-slate-200">
                  ...
                  <div className="flex gap-4 overflow-x-auto py-4 custom-scrollbar px-2">
                    {pages.map((p, i) => (
                      <button
                        ...
                        onClick={() => {
                          setRetargetSourceImage(
                            p.processedImage || p.originalImage || null,
                          );
                          setRetargetData({
                            sourceImage: p.processedImage || p.originalImage,
                          });
                        }}
                        className={`flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden border-4 transition-all ${retargetSourceImage === (p.processedImage || p.originalImage) ? "border-indigo-600 scale-110 shadow-lg" : "border-transparent opacity-40 hover:opacity-100"}`}
                      >
                        <img
                          src={p.processedImage || p.originalImage}
                          className="w-full h-full object-cover"
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center px-4">
                  <h3 className="text-xs font-black uppercase tracking-widest text-indigo-600">
                    Target (Where do they go?)
                  </h3>
                  <p className="text-[10px] font-black text-slate-400">
                    Target: Frame #{pages.indexOf(retargetPage) + 1}
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  <HotspotOverlay
                    image={
                      retargetPage.processedImage || retargetPage.originalImage!
                    }
                    ...
                    onRemoveHotspot={(label) =>
                      setRetargetData({
                        targetHotspots:
                          retargetPage.retargeting!.targetHotspots.filter(
                            (h) => h.label !== label,
                          ),
                      })
                    }
                    ...
                    {pages.map((p, i) => (
                      <button
                        ...
                        onClick={() => {
                          setActiveRetargetId(p.id);
                          setRetargetSourceImage(
                            p.retargeting?.sourceImage || null,
                          );
                        }}
                        className={`flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden border-4 transition-all ${activeRetargetId === p.id ? "border-indigo-600 scale-110 shadow-lg" : "border-transparent opacity-40 hover:opacity-100"}`}
                      >
                        <img
                          src={p.processedImage || p.originalImage}
                          className="w-full h-full object-cover"
            ...
            <div className="bg-white border-2 border-slate-100 rounded-[3rem] p-10 shadow-2xl space-y-6">
              <h3 className="text-xs font-black uppercase tracking-widest text-indigo-600 px-4">
              ...
              <div className="flex items-center gap-3 px-4">
                ...
                />
                <label
                  htmlFor="applyStyleCheck"
                  className="text-sm font-bold text-slate-700 cursor-pointer select-none"
                >
                  Enhance and completely restyle the image using the project's
                  Master Target Style
                </label>
              </div>
            </div>
      ...
      case "upload":
        return (
          <div className="max-w-5xl mx-auto py-24 px-8 space-y-12 text-center">
            ...
            <div
              onClick={() => !isProcessing && restyleInputRef.current?.click()}
              className={`aspect-video bg-white border-4 border-dashed border-slate-200 rounded-[6rem] flex flex-col items-center justify-center transition-all group shadow-inner ${isProcessing ? "cursor-not-allowed" : "cursor-pointer hover:border-indigo-600"}`}
            >
              {isProcessing ? (
                <div className="flex flex-col items-center gap-6">
                  <Loader2
                    ...
                    className="animate-spin text-indigo-600"
                  />
                  <p className="text-indigo-600 font-black uppercase tracking-[0.2em] text-3xl animate-pulse">
                    Extracting Elements...
                  </p>
                </div>
              ) : (
                <>
                  <Upload
                    size={100}
                    className="text-slate-200 group-hover:text-indigo-600 mb-10 transition-colors"
                  />
                  <p className="text-slate-400 font-black uppercase tracking-[0.2em] text-3xl group-hover:text-indigo-600 transition-colors">
                    Select Illustration Batch
                  </p>
                </>
              )}
              <input
            ...
            <button
              disabled={isProcessing}
              onClick={() => setCurrentStep("landing")}
              className="text-slate-400 font-bold hover:text-slate-600 underline text-xl disabled:opacity-50"
            >
              Cancel
            </button>
          </div>
      ...
      default:
        return (
          <div className="p-20 text-center font-black text-4xl">
            Module Not Found
          </div>
        );
    }
    ...
    <div className="min-h-screen flex flex-col bg-[#F8FAFC]">
      {isExtractingPdf && (
        <div className="fixed inset-0 min-h-screen bg-slate-900/80 backdrop-blur-md z-[100] flex flex-col items-center justify-center p-8">
          ...
          <h2 className="text-4xl font-black text-white uppercase tracking-widest text-center">
          ...
          <div className="w-full max-w-lg mt-8 bg-slate-800 rounded-full h-4 overflow-hidden relative">
            <div
              ...
              style={{
                width: `${extractionTotal > 0 ? (extractionProgress / extractionTotal) * 100 : 0}%`,
              }}
            ></div>
            {extractionTotal === 0 && (
          ...
          <p className="text-slate-400 font-bold mt-4 text-xl">
            {extractionTotal > 0
              ? `${extractionProgress} of ${extractionTotal} pages processed`
      ...
      {userMode === "professional" ? (
        <header className="h-36 bg-white/80 backdrop-blur-3xl border-b border-slate-100 sticky top-0 z-[60] px-20 flex items-center justify-between shadow-sm">
          ...
          >
            <div className="w-20 h-20 bg-indigo-600 rounded-[2.5rem] flex items-center justify-center text-white shadow-2xl group-hover:rotate-6 transition-all">
              <Sparkles size={40} />
            </div>
            <h1 className="text-5xl font-black tracking-tighter text-slate-900">
              StoryFlow <span className="text-indigo-600">Pro</span>
            </h1>
          </div>
          <div className="flex items-center gap-10">
            <div className="bg-slate-50 border border-slate-100 rounded-[2rem] px-12 py-5 flex items-center gap-12 shadow-inner">
              <input
                className="bg-transparent border-none outline-none font-black text-slate-800 text-2xl w-96"
                value={projectName}
                onChange={(e) => setProjectName(e.target.value)}
              />
              <div className="flex gap-4 items-center">
                {saveMessage && (
                  <span className="text-emerald-500 font-bold text-sm mr-2 animate-in fade-in slide-in-from-right-4">
                    {saveMessage}
                  </span>
                )}
                <button
                  onClick={handleStartNewProject}
                  className="text-indigo-600 p-4 bg-white rounded-2xl shadow-xl hover:scale-110 transition-transform"
                  title="New Project"
                >
                  <FilePlus size={28} />
                </button>
                ...
                <button
                  ...
                  className="text-emerald-600 p-4 bg-white rounded-2xl shadow-xl hover:scale-110 transition-transform"
                  title="Export Project"
                >
                  <FileDown size={28} />
                </button>
                <label
                  className={`text-emerald-600 p-4 bg-white rounded-2xl shadow-xl transition-transform ${isProcessing ? "opacity-50 cursor-not-allowed" : "hover:scale-110 cursor-pointer"}`}
            ...
            <button
              ...
              className="px-10 py-5 bg-emerald-50 text-emerald-600 rounded-[2rem] font-black text-sm uppercase tracking-widest flex items-center gap-4 border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all shadow-sm"
            ...
            {isAdmin && (
              <button
                ...
                className="px-10 py-5 bg-amber-50 text-amber-600 rounded-[2rem] font-black text-sm uppercase tracking-widest flex items-center gap-4 border border-amber-100 hover:bg-amber-600 hover:text-white transition-all shadow-sm"
                title="Manage Users"
              >
            ...
            <button
              onClick={() => setCurrentStep("characters")}
              className="px-10 py-5 bg-indigo-50 text-indigo-600 rounded-[2rem] font-black text-sm uppercase tracking-widest flex items-center gap-4 border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all shadow-sm"
            >
              <UserCheck size={24} /> SERIES CAST
            </button>
            <button
              onClick={() => setShowBibleEditor(!showBibleEditor)}
              className="p-5 bg-slate-900 text-white rounded-[2rem] shadow-2xl hover:scale-110 transition-all"
            >
              <Book size={32} />
            </button>
            <button
              onClick={() => setShowUserDashboard(true)}
              className="p-5 bg-indigo-50 text-indigo-600 rounded-[2rem] shadow-sm hover:bg-indigo-600 hover:text-white transition-all"
              title="User Profile"
            >
              <User size={32} />
            </button>
            <button
              onClick={logout}
              className="p-5 bg-rose-50 text-rose-600 rounded-[2rem] shadow-sm hover:bg-rose-600 hover:text-white transition-all"
              title="Sign Out"
            >
              <LogOut size={32} />
            </button>
        ...
        <header className="h-24 bg-white/80 backdrop-blur-3xl border-b border-slate-100 sticky top-0 z-[60] px-8 flex items-center justify-between shadow-sm">
          ...
          >
            <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-lg group-hover:rotate-6 transition-all">
              <Sparkles size={24} />
            </div>
            <h1 className="text-2xl font-black tracking-tighter text-slate-900">
              StoryFlow <span className="text-indigo-600">AI</span>
            </h1>
          </div>
          <div className="flex items-center gap-6">
            {userProfile && (
      ...
      {currentStep !== "landing" &&
        getAvailableSteps(settings.mode).length > 0 && (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-16">
          ...
          <div className="bg-white w-full max-w-4xl rounded-[5rem] p-20 shadow-2xl relative z-10 space-y-12 animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {savedProjects.length === 0 ? (
                <p className="text-slate-500 text-xl font-medium col-span-full text-center py-12">
                ...
                savedProjects.map((proj) => (
                  <div
                    key={proj.id}
                    className="bg-slate-50 rounded-[3rem] p-8 flex flex-col gap-6 shadow-sm border border-slate-100 hover:border-indigo-200 transition-colors group"
                  >
                    <div className="rounded-[2rem] overflow-hidden">
                      {renderProjectThumbnails(
                        proj,
                        "w-full h-48 flex flex-wrap bg-slate-200 group-hover:scale-[1.02] transition-transform origin-center",
                      )}
                    </div>
                    <div>
                    ...
                    <div className="flex gap-4 mt-auto">
                      <button
                        disabled={isProcessing}
                        onClick={() => handleLoadProject(proj)}
                        className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm hover:bg-indigo-700 transition-colors disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="p-4 bg-rose-100 text-rose-600 rounded-2xl hover:bg-rose-600 hover:text-white transition-colors disabled:opacity-50"
                      >
                        <X size={24} />
                      </button>
                    </div>
      ...
      {showBibleEditor && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-16">
          <div
            className="absolute inset-0 bg-slate-950/80 backdrop-blur-md"
            onClick={() => setShowBibleEditor(false)}
          />
          <div className="bg-white w-full max-w-3xl rounded-[5rem] p-20 shadow-2xl relative z-10 space-y-12 animate-in zoom-in duration-300">
            ...
            <textarea
              className="w-full h-[500px] bg-slate-50 border-none rounded-[3rem] p-12 text-sm font-medium outline-none resize-none shadow-inner leading-relaxed"
              value={settings.masterBible}
              onChange={(e) =>
                setSettings({ ...settings, masterBible: e.target.value })
              }
            ...
            <button
              onClick={() => setShowBibleEditor(false)}
              className="w-full py-10 bg-indigo-600 text-white rounded-[2.5rem] font-black text-3xl shadow-2xl hover:bg-indigo-500 transition-all"
            >
              LOCK BIBLE & CLOSE
            </button>
          </div>
