...
export const generateTextOnlyPDF = async (
  pages: BookPage[],
  format: ExportFormat,
  title: string,
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    if (page.isSpread && spreadMode === "WIDE_SPREAD") {
      ...
      if (page.originalText && page.textPositionOverride !== "hidden") {
        if (settings.spreadTextSide === "left") {
          drawTextWithPdfLib(
            spreadPage, spreadWidth, fullHeight, page.originalText,
      ...
      if (page.originalText && page.textPositionOverride !== "hidden") {
        if (settings.spreadTextSide === "left" || settings.spreadTextSide === "both") {
          const text = settings.spreadTextSide === "both" ? page.originalText.split("||")[0]?.trim() : page.originalText;
          drawTextWithPdfLib(
            leftPage, singleFullWidth, fullHeight, text || "",
        ...
        if (settings.spreadTextSide === "right" || settings.spreadTextSide === "both" || !settings.spreadTextSide) {
          const text = settings.spreadTextSide === "both" ? page.originalText.split("||").slice(1).join("\n\n")?.trim() : page.originalText;
          drawTextWithPdfLib(
            rightPage, singleFullWidth, fullHeight, text || "",
      ...
      if (page.originalText && page.textPositionOverride !== "hidden") {
        ...
        drawTextWithPdfLib(
          singlePage, singleFullWidth, fullHeight, page.originalText,
  ...
  a.href = url;
  a.download = `${title.replace(/[^a-z0-9]/gi, "_").toLowerCase()}_text_only.pdf`;
  a.click();
