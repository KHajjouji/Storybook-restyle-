...
/**
 * Builds the project-recovery metadata string that gets embedded in the
 * PDF's `subject` property.
 *
 * Two hard constraints, both of which previously caused
 * "Maximum call stack size exceeded":
 *  1. Strip every heavy / binary field (base64 images, long-form text).
 *     A multi-MB metadata string bloats the PDF and feeds jsPDF a giant
 *     string to process.
 *  2. Escape every non-ASCII character. jsPDF's internal `to8bitStream`
 *     only calls `String.fromCharCode.apply(undefined, hugeArray)` when
 *     the string contains chars > 0xFF (e.g. the smart quotes / em-dashes
 *     that are extremely common in children's-book text). Forcing the
 *     string to pure ASCII makes jsPDF take its safe early-return path.
 */
...
export const generateCoverPDF = async (
  ...
  // optimizeImageForPDF guarantees a JPEG data URI. Passing the JPEG data
  // URI string straight to jsPDF embeds the original bytes with no canvas
  // re-encode (no quality loss) and no PNG-decode call-stack risk.
...
/**
 * Composites an ordered list of image layers onto a 300 DPI canvas and
 * returns a JPEG data URI. The text layer is intentionally excluded by
 * the caller so that text can be embedded as a separate, selectable
 * vector layer in the PDF instead of being baked into the raster image.
 */
...
export const generateBookPDF = async (
  ...
  // Pre-load the user's chosen font for the in-browser preview. The PDF
  // itself falls back to helvetica, a standard built-in PDF font.
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    // A visible text layer means the AI generated text as its own layer.
    // We exclude it from the raster composite and add the text as real PDF
    // vector text instead, making it selectable / editable in any viewer.
    ...
    // Emit vector text when overlayText is explicitly enabled OR when the AI
    // produced a separate text layer (we replace that raster with real text).
