...
const optimizeImageForPDF = async (
  ...
  return new Promise<string>((resolve) => {
    ...
    img.onload = () => {
      const canvas = document.createElement("canvas");
      // Safety limit scaling down huge images if necessary to avoid stack explosion
      // though html-canvas-to-DataURL is robust.
...
export const generateCoverPDF = async (
  ...
  const finImgObj = await loadImage(finalCoverImage);
  pdf.addImage(
    finImgObj,
    "JPEG",
    0,
